...
{
    [Fact]
    public async Task HandleAsync_ClosesOwnedEvent()
    {
        var ownerUserId = Guid.CreateVersion7();
        var eventId = Guid.CreateVersion7();
        var now = new DateTimeOffset(2026, 7, 14, 12, 0, 0, TimeSpan.Zero);
        await using var dbContext = CreateDbContext();
        dbContext.Events.Add(CreateEvent(eventId, ownerUserId, uploadsEnabled: true));
        await dbContext.SaveChangesAsync();

        var handler = new CloseEventHandler(dbContext, new TestCurrentUser(ownerUserId), new TestTimeProvider(now));
        ...
        Assert.False((await dbContext.Events.SingleAsync(x => x.Id == eventId)).UploadsEnabled);
    }
    ...
    public async Task HandleAsync_ReturnsNotFoundForAnotherUsersEvent()
    {
        var eventId = Guid.CreateVersion7();
        await using var dbContext = CreateDbContext();
        dbContext.Events.Add(CreateEvent(eventId, Guid.CreateVersion7(), uploadsEnabled: true));
        await dbContext.SaveChangesAsync();

        var handler = new CloseEventHandler(
            dbContext,
    ...
    [Fact]
    public async Task HandleAsync_IsIdempotentWhenEventIsAlreadyClosed()
    {
        var ownerUserId = Guid.CreateVersion7();
        var eventId = Guid.CreateVersion7();
        var existingUpdatedAt = new DateTime(2026, 7, 10, 9, 0, 0, DateTimeKind.Utc);
        await using var dbContext = CreateDbContext();
        var eventEntity = CreateEvent(eventId, ownerUserId, uploadsEnabled: false);
        ...
        var handler = new CloseEventHandler(
            ...
            new TestTimeProvider(new DateTimeOffset(2026, 7, 14, 12, 0, 0, TimeSpan.Zero)));
